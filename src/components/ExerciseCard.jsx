...
export default function ExerciseCard({ exercise, i }) {
  const [completed, setsCompleted] = useState(0);
  function handleCompleted() {
    setsCompleted((completed + 1) % 6);
  }
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2">
        ...
        >
          ...
          <p className="font-medium">{completed}/5</p>
        </button>
      </div>
      <div className="flex flex-col bg-slate-950 gap-2 rounded">
